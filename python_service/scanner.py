...
class AdvancedScanner:
    def __init__(self, url, max_depth=3, max_urls=100, concurrency=10,timeout=100):
        self.start_url = url
        ...
        try:
            async with self.session.get(robots_url) as response:
                if response.status == 200:
                    ...
                    print(f"robots.txt found and parsed at {robots_url}")
                else:
    ...
    def is_allowed(self, url):
        if self.robot_parser is None:
            return True  # No robots.txt, so everything is allowed
